...
function boot() {
  print(`
    <div class="ascii">
 _       ___           __    ____  _           _ _
| |__   / _ \\_   _    / /   / ___|| |__   __ _| | | _____      __
| '_ \\ | | | | | | | / /   | \\___ \\| '_ \\ / _\` | | |/ _ \\ \\ / /
| | | || |_| | |_| |/ /     ___) | | | | (_| | | |  __/\\ V /
|_| |_| \\___/ \\__, /_/     |____/|_| |_|\\__,_|_|_|\\___| \\_/
              |___/
    </div>
  `);
...
function showStatus() {
  ...
  print(`<span class="dim">[NOTE]</span> repository links are editable placeholders except Tasbeeh Counter.`);
  print();
}
...
function openProject(name) {
  ...
  if (!p) {
    print(
      `<span class="yellow">The system cannot find the project specified:</span> ${esc(name)}`
    );
    print();
    return;
  }
...
function runCommand(raw) {
  ...
  switch (verb.toLowerCase()) {
    ...
    default:
      print(
        `'${esc(verb)}' is not recognized as an internal or external command, operable program or batch file.`
      );

      print(
        `Type <span class="yellow">help</span> for available commands.`
      );
